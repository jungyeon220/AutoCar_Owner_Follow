...
- 현재 배포 버전: `v0.9.1`
- 최신 패키지: `outputs/KNU_RC_DEVICE_POP_NEAREST_OWNER_v0.9.1.zip`

## 2026-07-16 — v0.8.0 ArUco 초기 인증 및 옷 패턴 재탐색

- Y 포즈 인증을 OpenCV ArUco `DICT_4X4_50`, 주인 ID 0 인증으로 교체했다.
- 최초 등록은 Bluetooth MAC, 화면 중앙 사람, 사람 영역 안의 ArUco 마커를 함께 확인한다.
- 5회 검사 중 3회 이상이며 평균 변 길이 50px 이상인 마커만 승인한다.
- 인증 후 ArUco 검출을 중지하고 2x3 HSV 옷 패턴과 CDS 밝기 보정값으로 추종한다.
- 주인 소실 시 즉시 정지하고 옷 패턴으로 최대 5초간 재탐색한다.
- 검색 실패 또는 후보 모호 판정 때만 ArUco 재인증으로 전환한다.
- Pose/trt_pose 런타임을 비활성화해 GPU 메모리와 인증 지연을 줄였다.
- 대시보드는 ArUco 상태, ID, 등록 진행률 및 검출 지연시간을 표시한다.
- 설치 시 인쇄용 `aruco_owner_id_0.png`를 자동 생성한다.
- 단위 테스트 33개와 v0.8.0 누적 설치 시뮬레이션을 통과했다.

남은 장비 검증은 `cv2.aruco` 존재 여부, 마커 실거리 인식, ArUco를 가린 뒤 추종 유지,
5초 옷 패턴 재탐색 및 복수 유사 의상 환경의 모호 판정이다.

### v0.8.1 설치 패키지 수정

- Windows ZIP의 역슬래시 내부 경로 때문에 Linux Python 3.6에서 파일을 열지 못하던 문제를 수정했다.
- 설치기가 정규화 경로와 실제 ZIP 멤버 이름을 매핑하도록 보강했다.
- 새 ZIP은 모든 내부 경로를 `/`로 저장하는 Python `zipfile` 방식으로 다시 생성했다.

### v0.8.2 640×640 및 최근접 마커 사용자 선택

- 카메라 입력 해상도와 YOLO 추론 해상도를 모두 640×640으로 변경했다.
- 추종 시작 시 화면 중앙 정렬 단계를 제거하고 즉시 ArUco 검색을 시작한다.
- 마커 중심과 사람 바운딩 박스 사이 거리가 가장 가까운 사람을 등록 후보로 선택한다.
- 동일 track ID가 5회 중 3회 이상 마커 최근접 후보일 때만 옷 패턴 등록을 승인한다.
- 인증 이후 ArUco 비활성화, 옷 패턴 추종 및 5초 재탐색 정책은 유지한다.

### v0.9.0 가장 가까운 사람 자동 등록 및 TILT 25°

- ArUco 및 Pose 인증을 모두 비활성화했다.
- 640×640 영상에서 사람 박스 높이가 가장 큰 대상을 가장 가까운 사람으로 간주한다.
- 같은 track ID가 5회 중 3회 이상 가장 가까운 후보이면 등록 대상을 잠근다.
- 후보 잠금 후 다른 사람이 더 크게 나타나도 등록 대상은 변경하지 않는다.
- 잠긴 대상의 옷 패턴과 CDS 밝기 보정 표본을 20프레임 수집한 뒤 추종한다.
- 등록 중 잠긴 대상이 1초 이상 사라지면 가장 가까운 사람 선택을 다시 시작한다.
- 추종 중 옷 패턴 재탐색이 5초간 실패하면 자동 재등록하지 않고 IDLE로 정지한다.
- 초기 카메라 위치를 PAN 90°, TILT 25°로 변경했다.

### v0.9.1 한백 POP 검출 복구 및 대시보드 TILT

- 장비 설정은 `backend=pop`이었지만 누적 패키지의 `vision.py`가 YOLOv5만 실행하던 회귀를 수정했다.
- 한백 예제와 동일하게 공유 `Pilot.Camera(320×320)`와 `Pilot.Object_Follow`를 사용한다.
- POP COCO 전체 결과에서 모든 `person` 박스를 공통 Detection 형식으로 변환한다.
- POP 카메라와 OpenCV 카메라가 동시에 CSI 장치를 열지 않도록 단일 공유 카메라로 구성했다.
- 카메라 및 추론 해상도를 320×320으로 복귀했다.
- 초기 PAN/TILT를 90°/0°로 변경했다.
- 인증 대시보드에 0~90° TILT 슬라이더와 적용 API를 추가했다.
- TILT 변경값은 즉시 적용되고 현재 런타임의 자동 추적 기준각도로 사용한다.

### GitHub 공개용 필수 파일 정리

- `GITHUB_UPLOAD/` 폴더를 만들고 최종 v0.9.1 소스와 공개 가능한 설정만 분리했다.
- 실행 소스, 설정 예시, 장비 업데이트 패키지, 설치기, 요구사항, 단위 테스트 및 개발로그를 포함했다.
- 구버전 ZIP, 중간 설치기, 실제 MAC 주소, 비밀번호, 실행 로그와 Python 캐시는 제외했다.
- GitHub 첫 화면에서 설치·실행·종료 방법을 확인할 수 있도록 전용 `README.md`를 작성했다.
- 이 통합 개발로그를 GitHub 배포본의 `DEVELOPMENT_LOG.md` 기준 문서로 지정했다.
- GitHub 배포 폴더의 최종 소스로 단위 테스트 40개를 통과했다.

### v0.9.1 실제 장비 기능 검증 결과

- POP에서 한 명과 여러 사람의 전체 바운딩 박스가 정상 검출됐다.
- 가장 가까운 사람 선택, 5회 중 3회 확인 및 등록 대상 잠금이 정상 동작했다.
- 옷 패턴 20프레임 등록 후 `FOLLOW_OWNER` 전환과 사용자 추종이 정상 동작했다.
- 다른 사람이 화면에 들어와도 등록 중인 대상과 추종 주인이 임의로 교체되지 않았다.
- 주인 소실 후 옷 패턴 기반 5초 재탐색, 검색 실패 정지 및 한국어 TTS가 정상 동작했다.
- 초기 TILT 0도, 대시보드 TILT 조절 및 자동 카메라 상하좌우 추적이 정상 동작했다.
- Bluetooth 연결 상태 표시와 연결 해제 시 즉시 정지가 정상 동작했다.
- LiDAR 거리 표시, 0.2m 정지 및 0.1m 비상정지가 정상 동작했다.
- 대시보드 영상·LiDAR 갱신, 속도 50~99 설정, 수동운전과 비상정지가 정상 동작했다.
- 장시간 연속 동작 안정성 검증을 제외한 기능 검증을 완료했다.
...
| 2026-07-16 | 인증 방식 개편, POP 복구 및 GitHub 정리 | v0.8.0~v0.9.1 구현, 장시간 동작 외 기능 검증 완료, 공개용 필수 파일 분리 |
...
### 2026-07-16 — 인증 방식 개편, POP 복구 및 배포 문서 정리
...
- GitHub 공개용 필수 파일을 `GITHUB_UPLOAD/`에 별도 구성
- 현재 통합 개발로그를 공개 저장소의 기준 개발로그로 지정

#### 현재 기준점

- 기준 버전: `v0.9.1`
- 기준 검출 방식: 공유 `Pilot.Camera`와 `Pilot.Object_Follow`
- 기준 등록 방식: 화면에서 가장 가까운 사람을 5회 중 3회 확인한 뒤 옷 패턴 등록
- 기준 해상도: 카메라 및 검출 320x320
- 기준 PAN/TILT: 90도/0도, 대시보드 TILT 조절 범위 0~90도
- 기준 Pose·ArUco 처리: 일반 운용에서 비활성화
- 다음 작업: 실제 장비에서 POP 복수 사람 검출, 등록, 추종 및 TILT 제어 연속 검증
...
- 한백 POP 카메라와 `Object_Follow`를 이용한 사람 검출
- 가장 가까운 사람 자동 선택 및 옷 패턴 기반 주인 등록
...
| POP | 설치 및 사용 가능 |
| ROS1 | 미설치, 사용하지 않음 |
| 사람 검출 | 한백 `pop.Pilot.Object_Follow`, COCO person 결과 사용 |
| YOLO | 로컬 YOLOv5 v6.0 호환 구현 보존, 현재 기본 경로에서는 미사용 |
| Pose | NVIDIA `trt_pose` ResNet18 224x224 호환 구현 보존, 현재 비활성화 |
| LiDAR | RPLIDAR 계열, `/dev/ttyUSB0`에서 동작 확인 |
| Bluetooth | BlueZ D-Bus, 등록 MAC 방식 |

Pose 모델 검증에 사용한 경로는 다음과 같다. 현재 v0.9.1 일반 운용에서는 Pose 모델을
로드하지 않는다.
...
```text
Pilot.Camera ──> POP Object_Follow ──> 전체 person Detection
                       │
                       ├─> 가장 큰 사람 박스 선택
                       ├─> 5회 중 3회 동일 후보 확인
                       └─> 옷 HSV 2x3 공간 패턴
...
| `autocar/adapters/vision.py` | POP 전체 person 결과 변환 및 YOLO 호환 경로 |
| `autocar/wave.py` | 과거 손 흔들기와 Y 포즈 판정기 보존, 현재 비활성화 |
...
1. 대시보드에서 추종 시작
2. Bluetooth, 카메라와 LiDAR 준비상태 확인
3. 화면에서 사람 박스 높이가 가장 큰 대상을 가장 가까운 사람으로 선택
4. 같은 후보가 5회 중 3회 확인되면 등록 대상을 잠금
5. 잠긴 사용자의 옷/CDS 패턴을 20프레임 수집해 프로필 확정
6. 카메라 기반 추종 시작
7. 주인을 잃으면 즉시 정지 후 5초간 검색
8. 검색 실패 시 TTS 안내 후 IDLE로 정지하고 대시보드 재시작 대기
...
유지하고 처리되지 못한 과거 프레임은 폐기한다. 공유 POP 카메라를 사용해 CSI 카메라를
두 번 여는 문제를 방지한다. Pose와 ArUco는 현재 일반 운용에서 실행하지 않는다.

## 6. 현재 가장 가까운 사람 등록 정책

초기 등록에는 손 흔들기, Y 포즈, ArUco 또는 얼굴인식을 사용하지 않는다. 사람 바운딩
박스 높이가 가장 큰 대상을 카메라에 가장 가까운 사람으로 간주한다.
...
- POP COCO 결과에서 `person` 클래스만 사용
- 한 프레임의 모든 사람 박스를 후보로 유지
- 화면 중심이 아니라 박스 높이 기준으로 최근접 후보 선택
- 5회 검사 중 같은 track ID가 3회 이상 선택되어야 대상 잠금
- 등록 중 다른 사람이 더 가까워져도 잠긴 대상은 교체하지 않음
- 잠긴 대상이 1초 이상 사라지면 선택 단계부터 다시 시작
- 대상은 가만히 있어도 등록 가능하며 별도 동작을 요구하지 않음
- 등록 완료 후 옷 패턴으로만 정상 추종 및 재탐색 수행

## 7. 사용자 프로필 등록

- 얼굴인식은 사용하지 않는다.
- 가장 가까운 사람으로 잠긴 대상의 상의 영역을 2x3 격자로 나눈다.
...
- 프로그램 시작 시 PAN 90도, TILT 0도 명령 전송
- 인증된 대시보드에서 TILT를 0~90도 범위로 변경 가능
- 대시보드에서 적용한 TILT가 현재 자동 상하 추적의 기준각도가 됨
...
| v0.8.0 | ArUco 초기 인증 후 옷 패턴 추종 및 5초 재탐색 |
| v0.8.1 | Linux에서 Windows ZIP 경로를 읽지 못하는 설치기 문제 수정 |
| v0.8.2 | 640x640 영상과 마커에 가장 가까운 사람 선택 |
| v0.9.0 | ArUco 제거, 가장 가까운 사람 자동 등록, 초기 TILT 25도 |
| v0.9.1 | POP Object_Follow 복구, 공유 320x320 카메라, 초기 TILT 0도와 대시보드 조절 |

## 11. 최신 배포 파일

| 파일 | SHA-256 |
|---|---|
| `outputs/KNU_RC_DEVICE_POP_NEAREST_OWNER_v0.9.1.zip` | `82631F172DD1F69EE73E73759D2937E7CDA2B25736DF91306A864EBE70D65DD7` |
| `outputs/install_device_nearest_owner.py` | `1BA68BDE7B6D0BB5ABAFA7A86F9579649ACBC6445D10BFE2BD6348EAEF0BCD51` |
...
installer = ROOT / "install_device_nearest_owner.py"
...
- Python 문법 검사
- JavaScript 문법 검사
- 공유 POP 카메라와 `Object_Follow` 전체 person 결과 변환 구현
- 가장 가까운 사람 5회 중 3회 선택 및 등록 대상 잠금 구현
- 초기 PAN/TILT 90도/0도와 대시보드 TILT 0~90도 제어 구현
- GitHub 공개 폴더에서 단위 테스트 40개 통과
- v0.9.1 누적 설치 시뮬레이션 통과
- 실제 POP 복수 사람 검출과 가장 가까운 사람 등록 확인
- 옷 패턴 등록, 추종, 5초 재탐색 및 검색 실패 정지 확인
- 카메라 PAN/TILT 추적과 대시보드 TILT 변경 확인
- Bluetooth 연결 해제 정지와 LiDAR 안전거리 정지 확인
- 한국어 TTS, 속도 제한, 수동운전 및 비상정지 확인
- 장시간 연속 동작을 제외한 전체 기능 정상 동작 확인

### 남은 필수 장비 검증

1. 30~60분 이상 연속 운전으로 카메라·검출 FPS, RSS, CUDA 메모리, 대시보드 지연 및 추종 반응속도가 장시간 유지되는지 확인
...
- 현재 기본 검출은 한백 POP이며 Pose와 ArUco는 비활성화 상태를 유지한다.
- POP와 OpenCV가 CSI 카메라를 동시에 열지 않도록 공유 카메라 구성을 유지한다.
